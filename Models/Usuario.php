<?php 
	...
	class Usuario extends Conexion
	{
		...
		{
			$this->abrir();
			$consulta = $this->conexion->prepare("SELECT usuarios.* FROM usuarios ");
		...
		{
			$this->abrir();
			$consulta = $this->conexion->prepare("SELECT usuarios.*, tiposusuario.nombre AS tipo FROM usuarios INNER JOIN tiposusuario ON usuarios.tipoUsuarioId = tiposusuario.id WHERE usuarios.id=?");
		...
		{
			$this->abrir();
			$consulta=$this->conexion->prepare("UPDATE usuarios SET cedula=?, nombre=?, apellido=?, telefono=?, contrasena=md5(?), tipoUsuarioId=?, usuario=? WHERE id=?");
			$consulta->bindParam(1, $this->cedula);
			$consulta->bindParam(2, $this->nombre);
			$consulta->bindParam(3, $this->apellido);
			$consulta->bindParam(4, $this->telefono);
